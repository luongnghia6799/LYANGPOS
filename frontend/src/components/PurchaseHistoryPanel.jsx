...
export default function PurchaseHistoryPanel({ partner, isOpen, onClose, onAddToCart }) {
    ...
    const fetchHistory = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`/api/orders?partner_id=${partner.id}&limit=10&type=Purchase`);
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[5000] flex justify-end">
                    ...
                    <m.div
                        ...
                        className="relative w-[400px] h-full bg-white dark:bg-slate-900 shadow-2xl flex flex-col border-l border-[#d4a574]/30"
                    >
                        {/* Header */}
                                    ...
                                    orders.map((order, idx) => {
                                        ...
                                        return (
                                            <m.div
                                                ...
                                                className="bg-gray-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-transparent hover:border-[#d4a574]/20 transition-all group"
                                            >
                                                <div className="flex justify-between items-start mb-2">
                                                    <div>
                                                        <div className="text-xs font-black text-[#d4a574] uppercase">#{order.display_id || order.id}</div>
                                ...
                                boughtProducts.length === 0 ? (
                                    ...
                                    boughtProducts.map((p, idx) => (
                                        <m.div
                                            ...
                                            animate={{ opacity: 1, y: 0 }}
                                            transition={{ delay: idx * 0.05 }}
                                            className="bg-gray-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-transparent hover:border-[#d4a574]/20 transition-all group flex items-center justify-between"
                                        >
